...
o próximo horário persistido em `profile/me` — sobrevive a um restart do container. Três camadas
de segurança contra mandar mensagem sem o aluno pedir (seção 5.6): só dispara com um `chat_id`
real já aprendido de uma mensagem recebida, nunca sem fila (silêncio em vez de spam) e nunca dois
lembretes seguidos sem resposta (`lembrete_sem_resposta`).
"""
...
class Agendador:
    ...
    async def _tick(self) -> None:
        perfil = await bloq(self._repo.obter_perfil)
        ...
        if perfil.proximo_lembrete is None:
            await self._agendar_proximo(perfil, agora_local)
            return
        ...
        sessao = await bloq(self._repo.obter_sessao)
        if sessao.estado != Estado.IDLE:
            return  # conversa em andamento: tenta de novo no próximo tick

        entradas = await bloq(self._repo.listar_entradas)
        ...
        await bloq(
            self._repo.salvar_perfil, perfil.model_copy(update={"lembrete_sem_resposta": True})
        )
        await self._router.iniciar_revisao()

        perfil_apos = await bloq(self._repo.obter_perfil)
        if perfil_apos is not None:
            await self._agendar_proximo(perfil_apos, agora_local)

    async def _agendar_proximo(self, perfil: Profile, agora_local: datetime) -> None:
        novo = proximo_horario(
            agora_local, perfil.lembretes_por_dia, perfil.janela_inicio, perfil.janela_fim
        )
        await bloq(
            self._repo.salvar_perfil,
